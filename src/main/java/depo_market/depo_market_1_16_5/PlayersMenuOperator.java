...
public class PlayersMenuOperator {
    ...
    public void MenuClick(int ClickedSlot, String disadvantage) {
        ...
        if (player_inv_state == INDEX_OF_MAIN_MENU) {
            ...
            if (ClickedSlot >= 0 & ClickedSlot < operator.dataBaseTradeItem.getTradeAmountList().size()) {
                if (operator.market.getPrice(operator.dataBaseTradeItem.getMenuSlotList().get(player_inv_state).getEnName()) * operator.dataBaseTradeItem.getTradeAmountList().get(ClickedSlot) < 50000) {
                ...
                if (operator.market.getPrice(operator.dataBaseTradeItem.getMenuSlotList().get(player_inv_state).getEnName()) * operator.dataBaseTradeItem.getTradeAmountList().get(ClickedSlot - 9) < 50000) {
    ...
    private void TradeItem(int TradeIndex, int AmountIndex, boolean isBuy) {
        final Material SelectedMaterial = operator.dataBaseTradeItem.getMenuSlotList().get(TradeIndex).getMaterial();
        final String SelectedEnName = operator.dataBaseTradeItem.getMenuSlotList().get(TradeIndex).getEnName();
        final int tradeAmount = operator.dataBaseTradeItem.getTradeAmountList().get(AmountIndex);
    ...
    private void Trade(boolean isBuy, ItemStack TradeItemStack, int tradeAmount, String SelectedEnName) {
        ...
        if (isBuy) {
            ...
            earnMoney = - operator.market.buy(SelectedEnName, tradedAmount) * 1.01f;//金額を計算。値段も変動
        } else {
            ...
            earnMoney = operator.market.sell(SelectedEnName, tradedAmount) * 0.99f;//金額を計算。値段も変動
        }
